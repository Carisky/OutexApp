 ...
 */
async function insertWorkout(knex, name, imageUrl) {
  try {
 ...
 * @param {string} name 
 */
async function insertRepeats(knex, workoutId, exerciseId, duration, repeats, dir, name) {
  try {
    ...
    console.error(`Error inserting repeat data for ${dir} - ${name}: ${error.message}`);
  }
}
...
exports.seed = async function(knex) {
  ...
  try {
    ...
    for (const dir of trainingsDirs) {
      ...
      await insertWorkout(knex, dir, path.join(initPath, dir, jpgFile));

      if (csvFile) {
        const csvFilePath = path.join(filesDirPath, csvFile);
        const parsedCsv = await readCsvFile(csvFilePath);
        ...
        for (const csvRow of parsedCsv) {
          ...
          if (workoutId && exerciseId) {
            await insertRepeats(knex, workoutId.id, exerciseId.id, Duration, Repeats, dir, Name);
          } else {
